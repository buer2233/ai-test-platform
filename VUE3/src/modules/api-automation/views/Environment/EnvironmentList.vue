<!--
  EnvironmentList.vue - 环境管理列表页面

  管理测试环境配置的完整页面，功能丰富：
  - 统计卡片：总环境数、已启用、已禁用、默认环境、收藏环境（点击可查看详情列表）
  - 搜索筛选：按名称、项目、状态搜索
  - 环境表格：名称（含收藏/默认图标）、描述、项目、Base URL、配置统计、状态开关
  - CRUD 操作：新建、编辑、查看详情、删除
  - 高级操作：复制环境、导出配置、设为默认、连接测试
  - 批量操作：全部启用/禁用、导出选中
  - 导入功能：从 JSON 文件导入或导入模板环境
  - 连接测试结果弹窗：展示请求/响应头、响应体、错误信息
  - 配置预览弹窗：查看和编辑 Headers/Variables
  - 统计数据弹窗：按分类展示环境列表，支持弹窗内编辑/测试/删除
-->
<template>
