<!--
  Dashboard/index.vue - 测试报告仪表盘

  接口自动化测试的数据概览页面，包含以下功能：
  - 顶部筛选栏：按项目、集合、负责人、模块、时间范围多维度筛选
  - 统计概览卡片：总用例数、通过用例、失败用例、通过率（可点击查看详情）
  - ECharts 图表区域：柱状图展示各状态用例数量、饼图展示状态分布占比
  - 测试结果列表：支持状态过滤、分页浏览、跳转详情
  - 数据详情弹窗（DataDetailDialog）：按状态分类展示详细数据
  - 30 秒自动刷新机制，保持数据实时性
  - 图表支持点击交互，点击柱/饼块可弹出对应状态的详情弹窗
-->
<template>
