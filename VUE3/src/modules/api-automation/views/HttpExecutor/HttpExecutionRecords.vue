<!--
  HttpExecutionRecords.vue - HTTP 执行记录管理页面

  管理和查看所有 HTTP 请求的执行记录，支持以下功能：
  - 统计卡片：总记录数、成功数、失败数、超时数、错误数、收藏数（点击卡片可快速筛选）
  - 多维度筛选：日期范围、执行状态、执行来源（手动/定时/批量/API/直接HTTP）
  - 关键字搜索：按 URL 或错误信息模糊搜索
  - 数据表格：请求方法、URL、状态码、执行状态、响应时间(颜色分级)、响应大小、来源、时间
  - 收藏功能：切换单条记录的收藏状态
  - 详情弹窗：通过 HttpExecutionRecordDetail 组件展示完整执行详情
  - 批量操作：批量删除选中记录、批量导出（开发中）
  - 按条件删除：按状态和/或日期条件批量删除记录（不可恢复，带警告提示）
  - 分页：支持自定义每页条数
-->
<template>
