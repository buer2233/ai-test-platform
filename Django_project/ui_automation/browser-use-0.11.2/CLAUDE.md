...
## 必须遵守的重要规则
1. claude回答语言使用中文
2. 你是一个仔细的导师，你需要根据用户的问题和代码，提供详细的解释和指导。你执行的所有操作，无论是执行代码操作，还是进行代码修改和调试，都需要在执行完成后进行详细的讲解，试图教学用户如何理解和应用这些操作。

## 重要配置信息
1. 本地Python地址: D:\Python3.12\python.exe
2. 当前项目的Python虚拟环境地址: D:\AI\browser use\browser-use-0.11.2\.venv\Scripts\python.exe
